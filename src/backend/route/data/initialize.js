...
router.route('/data/initialize')
    ...
    .get((request, response, next) => {
        ...
        knex.transaction((trx) => {
            return trx('scheduleSystem.dbo.jobType').select('*').where({ active: 1 }).orderBy('displaySequence').debug(false)
                .then((resultset) => {
                    ...
                    return trx('scheduleSystem.dbo.productType').select('*').where({ active: 1 }).orderBy('displaySequence').debug(false);
                }).then((resultset) => {
                    initData.productType = resultset;
                    return trx('scheduleSystem.dbo.phase').select('*').where({ active: 1 }).orderBy('displaySequence').debug(false);
                });
        }).then((resultset) => {
            initData.phase = resultset;
            return response.status(200).json(initData);
        }).catch((error) => {
            return response.status(500).json(endpointErrorHandler(request.method, request.originalUrl, `系統初始化資料讀取失敗: ${error}`));
        }).finally(() => {
            knex.destroy();
        });
    });

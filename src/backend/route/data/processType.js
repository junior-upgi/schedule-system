...
router.route('/data/processTypes/id/:id')
    .all(tokenValidation)
    ...
    .patch((request, response, next) => { // update 'reference' field of a particular record
        ...
        if ((reference === undefined) || (reference === null) || (reference === '')) {
            ...
            let knex = require('knex')(mssqlConfig);
            knex('scheduleSystem.dbo.processType').update({ reference: reference }).where({ id: id }).debug(false)
                .then(() => {
    ...
    .delete((request, response, next) => { // deactivate target record and reorder (deprecated=null is assumed to be true)
        ...
        knex.transaction((trx) => {
            ...
            return trx('scheduleSystem.dbo.processType').select('displaySequence').where({ id: id }).debug(false)
                .then((resultset) => {
                    targetRecordDisplaySequence = resultset[0].displaySequence;
                    // deactivate the target record
                    return trx('scheduleSystem.dbo.processType').update({
                    ...
                    return trx.raw('UPDATE scheduleSystem.dbo.processType SET displaySequence=displaySequence-1 WHERE active=1 AND displaySequence>?;', [targetRecordDisplaySequence]).debug(false);
                }).then(() => { // get a fresh set of processType data
...
router.route('/data/processTypes/id/:id/displaySequence/:displaySequence')
    .all(tokenValidation)
            ...
            return trx('scheduleSystem.dbo.processType').select('displaySequence').where({ id: id }).debug(false)
                .then((resultset) => {
                    ...
                    if (targetRecordDisplaySequence < request.params.displaySequence) {
                        ...
                        // note: raw query was used due to an issue of getting standard knex query to work
                        return trx.raw('UPDATE scheduleSystem.dbo.processType SET displaySequence=displaySequence-1 WHERE active=1 AND displaySequence>? AND displaySequence<=?;', [targetRecordDisplaySequence, request.params.displaySequence]).debug(false);
                    } else if (targetRecordDisplaySequence > request.params.displaySequence) {
                        // adjust displaySequence of all affected active records
                        // note: raw query was used due to an issue of getting standard knex query to work
                        return trx.raw('UPDATE scheduleSystem.dbo.processType SET displaySequence=displaySequence+1 WHERE active=1 AND displaySequence>=? AND displaySequence<?;', [request.params.displaySequence, targetRecordDisplaySequence]).debug(false);
...
router.route('/data/processTypes')
    .all(tokenValidation)
    .get((request, response, next) => { // get all active records
        let knex = require('knex')(mssqlConfig);
        knex('scheduleSystem.dbo.processType')
            .select('*').where({ active: 1, deprecated: null })
            .orderBy('displaySequence').debug(false)
            .then((resultset) => {
                return response.status(200).json(resultset);
...
router.route('/data/processTypes/inactive')
    .all(tokenValidation)
    .get((request, response, next) => { // get all inactive but not deprecated records
        let id = request.params.id;
        let knex = require('knex')(mssqlConfig);
        knex('scheduleSystem.dbo.processType').select('*').where({ active: 0, deprecated: null }).debug(false)
            .then((resultset) => {
                return response.status(200).json(resultset);
            }).catch((error) => {
                return response.status(500).json(
...
router.route('/data/processTypes/inactive/id/:id')
    .all(tokenValidation)
...
router.route('/data/processTypes/all')
    .all(tokenValidation)
    .get((request, response, next) => { // get all records
        let id = request.params.id;
        let knex = require('knex')(mssqlConfig);
        knex('scheduleSystem.dbo.processType').select('*')
